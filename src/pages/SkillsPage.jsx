...
export default function SkillsPage() {
  ...
  return (
    <div className="page-wrap">
      ...
      <div className="skills-page-wrap">
        <div className="skills-bg" aria-hidden="true">SKILL</div>

        <h2 className="skills-heading reveal">
          What I<br />actually <span className="accent">know.</span>
        </h2>

        <div className="skills-list">
          {skills.map(({ name, pct, pills }, i) => (
            <div key={i} className="skill-row reveal" style={{ transitionDelay: `${i * 0.08}s` }}>
        ...
        <div className="tools-block reveal d3">
          ...
          <div className="tools-list">
            {certs.map(c => <span key={c} className="tool-tag cert">{c}</span>)}
          </div>
        </div>

        {/* disciplines */}
        <div className="skills-disciplines reveal" style={{ marginTop: "4rem" }}>
          <div className="tools-label">// Disciplines</div>
          <div className="disciplines-grid">
            {[
              { name: "Decision Analytics",    desc: "Translating data into decisions that hold up under real conditions." },
              { name: "Market Entry Strategy", desc: "CBA, sensitivity modelling, phased pilot frameworks." },
              { name: "Demand Forecasting",    desc: "Time series, exponential smoothing, scenario testing." },
              { name: "Product Management",    desc: "Roadmapping, GTM, stakeholder alignment, shipping." },
              { name: "Stakeholder Comms",     desc: "Cross-functional reporting, decks, real-world briefs." },
              { name: "Event Operations",      desc: "End-to-end planning, sponsorship, execution under pressure." },
            ].map(({ name, desc }) => (
              <div key={name} className="discipline-item">
